...
CREATE INDEX IF NOT EXISTS track_audio_profiles_source_idx
  ON track_audio_profiles (source_run_id, track_id);

CREATE TABLE IF NOT EXISTS audio_temporal_segments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS audio_temporal_segments_track_idx
  ON audio_temporal_segments (profile_id, segment_index);

CREATE TABLE IF NOT EXISTS audio_modes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  profile_id uuid NOT NULL REFERENCES track_audio_profiles(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS audio_modes_track_idx
  ON audio_modes (profile_id, mode_index);

CREATE TABLE IF NOT EXISTS audio_mode_intervals (
