  ...
  window.createSplineLayer = function createSplineLayer(gl, canvas) {
    ...
    const waveProg = link(
      ...
      `#version 300 es
       ...
       uniform float visualZoom;
       uniform float layerVerticalScale;
       ...
       void main() {
         ...
         gl_Position = vec4(p.x * visualZoom, p.y * visualZoom * layerVerticalScale, p.z, 1.0);
         vPos = p;
       }`,
      `#version 300 es
       precision highp float;
       ...
       void main() {
         ...
         oColor = vec4(layerColor, F * opacity * brightness);
       }`
    );
    ...
    const waveU = {
      ...
      visualZoom: uloc(gl, waveProg, 'visualZoom'),
      layerVerticalScale: uloc(gl, waveProg, 'layerVerticalScale'),
      ...
      layerColor: uloc(gl, waveProg, 'layerColor'),
    };
    ...
    function render(timeSec) {
      ...
      gl.uniform1f(waveU.visualZoom, settings.visualZoom || 1.0);
      ...
      const amplitudes = settings.layerAmplitudes || [1.0];
      const offsets = settings.layerTimeOffsets || [0.0];
      const colors = settings.layerColors || [[1.0, 1.0, 1.0]];
      gl.bindVertexArray(grid.vao);
      for (let layer = 0; layer < amplitudes.length; layer++) {
        const depth = amplitudes.length > 1 ? layer / (amplitudes.length - 1) : 0.5;
        gl.uniform1f(waveU.time, timeSec + (offsets[layer] || 0));
        gl.uniform1f(waveU.waveHeightScale, settings.waveHeightScale);
        gl.uniform1f(waveU.layerVerticalScale, amplitudes[layer]);
        gl.uniform1f(waveU.ffdYAmp, settings.ffdYAmp * (0.78 + depth * 0.42));
        gl.uniform1f(waveU.ffdZAmp, settings.ffdZAmp * (1.18 - depth * 0.34));
        gl.uniform1f(waveU.opacity, settings.opacity * (layer === 1 ? 0.48 : 0.32));
        const color = colors[layer] || colors[0];
        gl.uniform3f(waveU.layerColor, color[0], color[1], color[2]);
