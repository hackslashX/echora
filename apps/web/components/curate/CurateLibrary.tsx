...
import { ArrowLeft, Clock3, Disc3, ListMusic, MessageSquareText, Pause, Play, RefreshCw, Route, Save, Trash2, WandSparkles, X } from "lucide-react";
...
export default function CurateLibrary() {
  ...
  function openCuration(curation: Curation) {
    ...
    setActiveAspect(curation.curation_type === "examples" ? "examples" : curation.curation_type === "time_of_day" ? "time_of_day" : curation.curation_type === "sonic_journey" ? "sonic" : "language");
  ...
  useDialogFocus(deleteDialogRef, Boolean(deleteTarget), () => { if (!busy) setDeleteTarget(null); });
  ...
  return <AppShell title="Curate" footer={<CopyrightFooter />} breadcrumb flush fullPage grid={{ columns, rows: [1] }}>
    <main className={styles.page} style={{ gridTemplateColumns: trackTemplate(columns, 160), gridTemplateRows: trackTemplate([1], 88) }}>
      ...
      <section className={`${styles.recipe} ${mobilePane === "recipe" ? `${styles.mobileActive} ${paneTransition}` : ""}`}>
        ...
        {activeAspect === "sonic" && <section className={styles.aspectPanel}><Alert tone="warning" title="Sonic journeys ignore the other tabs"><b>Language</b>, <b>Like / not like</b>, and <b>Time of day</b> settings do not apply here. A journey is shaped only by its waypoints and the lyrics slider. Switch back to another tab to use those signals.</Alert><TrackReferencePicker label="Start · where the journey begins" value={journeyStart} onChange={setJourneyStart} single /><TrackReferencePicker label="Stops · intermediate vibes, in order" value={journeyStops} onChange={setJourneyStops} /><TrackReferencePicker label="End · where it lands" value={journeyEnd} onChange={setJourneyEnd} single /><div className={styles.familiarity}><header><span>Lyrics influence</span><b>{100 - journeyLyricsWeight}% sound · {journeyLyricsWeight}% lyrics</b></header><input type="range" min="0" max="60" step="5" value={journeyLyricsWeight} onChange={event => setJourneyLyricsWeight(Number(event.target.value))} aria-label="Lyrics influence" /><small>Waypoints are connected through MuLan and MERT sound space, with lyric themes steering the path between stops.</small></div><p className={styles.aspectHint}>The playlist walks from the start track through each stop, in order, to the end. Sound (MuLan + MERT) shapes the route; the slider blends in lyrical meaning.</p></section>}
        ...
        <details className={styles.optionalSettings}><summary>Listening mix and refresh settings</summary><div className={styles.commonSettings}>
