...
export default function SyncLibrary() {
  ...
  return <AppShell title="Sync" footer={<CopyrightFooter />} breadcrumb flush fullPage grid={{ columns, rows }}>
    <main className={styles.page} style={{ gridTemplateColumns: trackTemplate(columns, 180), gridTemplateRows: trackTemplate(rows, 152) }}>
      <section className={styles.intro}><h1>Sync,<br /><em>step by step.</em></h1><p>Sync reads your server without changing its library. Echora keeps existing analysis and processes only the work each track still needs.</p><ol className={styles.steps}><li><b>1</b><div><strong>Read the catalog</strong><small>Load track IDs, metadata, artwork references, and the current library count from Navidrome.</small></div></li><li><b>2</b><div><strong>Resolve track identity</strong><small>Stream the source audio and use its SHA-256 content hash to link duplicates without merging recordings.</small></div></li><li><b>3</b><div><strong>Analyze missing representations</strong><small>MuQ-MuLan maps musical meaning, MERT measures acoustic structure, and BGE-M3 maps lyric language and themes. Demucs and MELODIA retain vocal, instrumental, and full-mix melody contours for hum search. FA-Kara converts existing time-synced lyrics into syllable-level karaoke timing.</small></div></li><li><b>4</b><div><strong>Commit the results</strong><small>Store embeddings, karaoke lyrics, and provenance, then make the tracks available to Browse, Galaxy, concepts, journeys, and the player.</small></div></li></ol><div className={styles.server}><Server /><div><small>Connected server</small><strong>{status?.server || "Reading connection"}</strong></div></div></section>
