...
export default function SyncLibrary() {
  ...
  return <AppShell title="Sync" footer={<CopyrightFooter />} breadcrumb flush fullPage grid={{ columns, rows }}>
    <main className={styles.page} style={{ gridTemplateColumns: trackTemplate(columns, 180), gridTemplateRows: trackTemplate(rows, 152) }}>
      <section className={styles.intro}><h1>Sync,<br /><em>step by step.</em></h1><p>Sync reads your server without changing its library. Echora keeps existing analysis and processes only the work each track still needs.</p><ol className={styles.steps}><li><b>1</b><div><strong>Read the catalog</strong><small>Load track IDs, metadata, artwork references, and the current library count from Navidrome.</small></div></li><li><b>2</b><div><strong>Resolve track identity</strong><small>Stream the source audio and use its SHA-256 content hash to link duplicates without merging recordings.</small></div></li><li><b>3</b><div><strong>Analyze missing representations</strong><small>Run MuQ-MuLan for semantic meaning, MERT for acoustic structure, and BGE-M3 for available lyrics when any representation is absent.</small></div></li><li><b>4</b><div><strong>Commit the results</strong><small>Store embeddings and provenance, then make the tracks available to Browse, Galaxy, concepts, and journeys.</small></div></li></ol><div className={styles.server}><Server /><div><small>Connected server</small><strong>{status?.server || "Reading connection"}</strong></div></div></section>
      <section className={styles.workspace}>
        ...
        {active || job?.status === "complete" || job?.status === "failed" ? <section className={styles.progress}><div><span>{job?.phase?.toUpperCase()}</span><strong>{job?.message || (job?.track ? `${job.track.artist || "Unknown artist"} · ${job.track.title}` : "Preparing models")}</strong><small>{job?.completed || 0} of {job?.total || 0} {job?.unit || "tracks"}</small></div><b>{percent}%</b><i><u style={{ width: `${percent}%` }} /></i>{job?.status === "failed" && <p>{job.error}</p>}{job?.status === "complete" && <div className={styles.summary}><span>{job.summary?.inserted || 0} new</span><span>{job.summary?.already_linked || 0} reused</span><span>{job.summary?.failed || 0} failed</span><span>{job.summary?.lyrics_embedded || 0} lyrics embedded</span><span>{job.summary?.unlinked || 0} unlinked</span></div>}</section> : <section className={styles.ready}><div className={styles.mode}><button className={mode === "all" ? styles.selected : ""} onClick={() => setMode("all")}><strong>ENTIRE LIBRARY</strong><small>Verify every track and fill missing audio and lyrics representations</small></button><button className={mode === "missing" ? styles.selected : ""} onClick={() => setMode("missing")}><strong>NEW TRACKS ONLY</strong><small>Process the {status?.missing || 0} tracks not yet indexed</small></button></div><button className={styles.start} onClick={start} disabled={busy || !status || (mode === "missing" && status.missing === 0)}>START PROCESSING <b>↗</b></button></section>}
