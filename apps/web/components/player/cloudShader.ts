...
void main() {
  ...
  vec3 cloudLight=mix(vec3(.78,.81,.84),pigment,.68);
  ...
  for(int i=0;i<56;i++) {
    float t=(float(i)+.5+rayJitter)*.08;
    ...
    smoke += pigment*(.009+shade*.014+rim*.022);
    smoke += cloudLight*rim*(.08+treble*.19);
    smoke += cloudLight*pow(shade,2.0)*mid*.14;
    smoke *= .95+bass*.15;
