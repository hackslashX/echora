...
No schema migration or new dependency is required. Migration 037's JSONB cache already stores a revision, source duration and hop. `VISUAL_FEATURE_REVISION = "2"` is imported by the API and `processing_plan.plan_audio`; revision-1 rows are pending, never served as revision 2. Normal audio processing regenerates only the visual cache when other analysis contracts are current. The existing 22050 Hz mono prerequisite is reused. A successful upsert atomically replaces the old cache; failed extraction rolls back. Processing does not delete model results.
