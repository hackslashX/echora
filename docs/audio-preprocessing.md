...
The experimental Roformer repository lacks an explicit source license grant, and checkpoint redistribution permission has not been established. Vendored source includes provenance in `vendor/roformer/NOTICE`. Resolve source and weight licensing before distributing a release containing this implementation; attribution alone is not permission.
