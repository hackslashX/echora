...
The Hugging Face model repository declares MIT in its [model card at the pinned revision](https://huggingface.co/KimberleyJSN/melbandroformer/blob/ac9b0614ab3cd7f77219e18ba494dfd93956c348/README.md). The separate GitHub repository supplying the copied architecture code has no license file at its pinned revision. Resolve that source-code license before distributing a release containing this implementation; the model's license declaration does not establish the license of separately sourced code. `vendor/roformer/NOTICE` records both sources and the distinction.
