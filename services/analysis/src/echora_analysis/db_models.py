...
class Curation(Base):
    ...
    journey_start_track_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tracks.id", ondelete="SET NULL"))
    journey_stop_track_ids: Mapped[list[uuid.UUID]] = mapped_column(ARRAY(UUID(as_uuid=True)), default=list)
    journey_end_track_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tracks.id", ondelete="SET NULL"))
    journey_lyrics_weight: Mapped[int] = mapped_column(Integer, default=25)
